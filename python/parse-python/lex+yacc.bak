(module parser racket
  ...
  (define (mk-type-eassign t lhs rhs)
     (match rhs
       [(input 'i) (input lhs)]
       [(input s)  (list (input-with-type t lhs) (sprint s))]
       [x (eassign lhs rhs)]))
  ...
  (define-lex-abbrevs
   ...
   (special-keyword (:or "and"      "del"       "for"       "is"        "raise"
                         ...
                         "def"       "finally"   "in"        "print"  "int" "float" "double"
...
#;(define (build-primary expr subs start)
  ...
  (define-empty-tokens et
    (!= % %= & &= |(| |)| * ** **= *= + += |,| - -= |.| / // //= /= : |;|
     ...
     in is int float double lambda not or pass print math.sqrt range pow raise return try while math yield |{| \| \|= |}| ~ EOF))
  (define p
    (parser
...
;       ((compound_stmt NEWLINE) $1))
     ...
     (testlist_list_plus ((tuple_or_test)  $1)
                          ...
                          ((tuple_or_test = type |(| testlist_list_plus |)|) (mk-type-eassign $3 $1 $5)))
     (type ((int)  (value "int"))
           ((float)  (value "float"))
           ((double)  (value "double")))
